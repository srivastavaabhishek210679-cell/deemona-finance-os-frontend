// rebuild-app.js
// Run: node rebuild-app.js
// Writes a new enterprise-grade App.jsx with light theme

const fs = require('fs');
const path = require('path');

const target = path.join('C:\\', 'deemona-finance-os', 'frontend', 'src', 'App.jsx');

// Read current file to extract dynamic content (routes with all page imports)
const current = fs.readFileSync(target, 'utf8');

// Extract all import lines
const importLines = current.split('\n').filter(l => l.startsWith('import')).join('\n');

const newApp = `import { useState, useEffect, useRef } from 'react';
...
fs.writeFileSync(target, newApp, 'utf8');
console.log('App.jsx written successfully');
console.log('Lines:', newApp.split('\n').length);
