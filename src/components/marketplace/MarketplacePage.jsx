...
const TIER_COLORS = { core:'#22C98A', premium:'#1B4FD8', enterprise:'#F5A623' };
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
          ...
          { label: 'Premium Agents', value: stats.premium || 0,    color: '#1B4FD8', sub: 'Advanced AI' },
            ...
            <div key={agent.id} style={{ borderRadius: 14, border: `1px solid ${agent.enabled ? '#1B4FD840' : 'var(--border)'}`, padding: 20, background: agent.enabled ? '#1B4FD806' : 'var(--surface-2)', transition: 'all 0.2s' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: agent.enabled ? 'linear-gradient(135deg,#1B4FD8,#3B82F6)' : 'var(--surface-3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>{agent.icon}</div>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 700 }}>{agent.name}</div>
                    <div style={{ display: 'flex', gap: 6, marginTop: 3 }}>
                      <span style={{ padding: '1px 7px', borderRadius: 100, fontSize: 10, fontWeight: 700, background: (TIER_COLORS[agent.tier]||'#1B4FD8')+'20', color: TIER_COLORS[agent.tier]||'#1B4FD8' }}>{agent.tier?.toUpperCase()}</span>
                      <span style={{ padding: '1px 7px', borderRadius: 100, fontSize: 10, fontWeight: 600, background: (STATUS_COLORS[agent.status]||'#1B4FD8')+'20', color: STATUS_COLORS[agent.status]||'#1B4FD8' }}>{agent.status}</span>
                    </div>
                  </div>
                </div>
                {/* Toggle switch */}
                <button onClick={() => toggle(agent)} disabled={toggling === agent.id} style={{ width: 48, height: 26, borderRadius: 13, background: agent.enabled ? '#1B4FD8' : 'var(--surface-3)', border: 'none', cursor: toggling === agent.id ? 'not-allowed' : 'pointer', position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}>
              ...
              <div style={{ marginBottom: 14 }}>
                {agent.features?.slice(0, 3).map((f, i) => (
                  <div key={i} style={{ fontSize: 12, color: 'var(--text-secondary)', padding: '2px 0', display: 'flex', gap: 6 }}>
                    <span style={{ color: agent.enabled ? '#1B4FD8' : 'var(--text-muted)' }}>›</span>{f}
                  </div>
                ))}
              </div>
