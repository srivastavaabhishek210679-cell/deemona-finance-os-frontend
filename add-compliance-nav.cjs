const fs = require('fs');
const f = 'C:/deemona-finance-os/frontend/src/App.jsx';
let c = fs.readFileSync(f, 'utf8');

if (!c.includes("'Compliance Docs'") && !c.includes('"Compliance Docs"')) {
  c = c.replace(
    "{ path: '/corporate-docs', label: 'Corporate Docs',",
    "{ path: '/compliance-docs', label: 'Compliance Docs', icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },\n      { path: '/corporate-docs', label: 'Corporate Docs',"
  );
  fs.writeFileSync(f, c, 'utf8');
  console.log('Nav added');
} else {
  console.log('Already exists');
}

// Also verify route exists
console.log('Has compliance-docs route:', c.includes("path: '/compliance-docs'"));
console.log('Has Batch2DocsPage import:', c.includes('Batch2DocsPage'));
