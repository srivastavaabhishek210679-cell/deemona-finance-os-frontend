...
// 1. Create WhiteLabelPage component directly
const wlDir = FRONTEND.replace(/\//g,'\\') + '\\components\\admin';
if (!fs.existsSync(wlDir)) fs.mkdirSync(wlDir, { recursive: true });

const wlContent = `import { useState } from 'react';
...
fs.writeFileSync(wlDir + '\\WhiteLabelPage.jsx', wlContent, 'utf8');
console.log('✓ WhiteLabelPage.jsx created');
...
if (!app.includes("label: 'White Label'")) {
  app = app.replace(
...
console.log('\nDone! WhiteLabelPage:', app.includes('WhiteLabelPage'), '| Route:', app.includes("'/white-label'"));
